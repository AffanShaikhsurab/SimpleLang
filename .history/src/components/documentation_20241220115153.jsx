      ...
      'error-handling': {
        ...
        content: (
          <DocSection 
            ...
            description="Sometimes things go wrong, but we can handle it!"
          >
            <SubSection title="Try and Oops">
              <CodeExample 
                title="Error Handling"
                code={`// Basic error handling
  try
      number is "hello" + 5   // This will cause an error
  oops
      show("Oops! Can't add text and numbers!")
  .
  
  // Stopping with a message
  try
      if score < 0 then
          stop with "Score can't be negative!"
      .
...
const DocSection = ({ title, description, children }) => (
    <div className="mb-16 px-6 py-8 bg-white shadow-lg rounded-lg border border-gray-200">
      <h2 className="text-4xl font-extrabold text-gray-800 mb-4">{title}</h2>
      {description && <p className="text-lg text-gray-600 leading-relaxed mb-6">{description}</p>}
  ...
  const CodeExample = ({ code, title }) => (
    <div className="relative rounded-lg overflow-hidden shadow-lg border border-gray-300 my-6">
      {/* Gradient Background Effect */}
      <div className="absolute inset-0 -z-10">
        <div className="absolute top-0 right-0 h-[300px] w-[300px] translate-x-[20%] -translate-y-[20%] rounded-full bg-[rgba(173,109,244,0.5)] opacity-50 blur-[120px]"></div>
        <div className="absolute bottom-0 left-0 h-[300px] w-[300px] -translate-x-[20%] translate-y-[20%] rounded-full bg-[rgba(244,109,173,0.5)] opacity-50 blur-[120px]"></div>
      </div>
      
      {/* Title Section */}
      {title && (
        <div className="px-4 py-3 bg-gray-100 border-b border-gray-300 text-sm font-semibold text-gray-800">
          {title}
        </div>
      )}
